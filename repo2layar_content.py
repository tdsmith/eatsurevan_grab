...
def main():
    ...
    for rguid, resto in repo.items():
        ...
        for i in [j[1] for j in sorted([ (vch2datetime(resto['inspections'][k]['date']), k) for k in xrange(len(resto['inspections'])) ])]:
                print >> f, '<h2>%s</h2>' % resto['inspections'][i]['date']
                for finding in resto['inspections'][i]['findings']:
                    print >> f, ('<h4>%s</h4><p>%s</p>' % (finding[0], finding[1])).encode('ascii', 'xmlcharrefreplace')
        f.close()
